    ...
    if(tags = container.attr('data-tags')){
      ...
      $(columns).each( ( index, element ) => {
        ...
        $(headingsNoticed).each( ( index, element ) => {
          //$href_parsed = urlencode( preg_replace('/[^a-zA-Z0-9_ %\[\]\.\(\)%&-]/s', '', wp_strip_all_tags(do_shortcode($value)))  );
          let strippedTagsHeadingText = $(element).text().replace(/(<([^>]+)>)/gi, ""); //remove html signs,
          let urlencodedText = strippedTagsHeadingText.replace(/\W/g,'_').replace(/\_/g, '-'); //change all not word boundaries to _ (\W does not catch _ sign), then _ to -

          //if last letter was entity, remove remaining - sign
          if ( urlencodedText[urlencodedText.length-1] === '-' ) {
            urlencodedText = urlencodedText.slice(0, -1);
          }

          if( $(element).attr('id') ) { //replace toc_X with user assigned ID

            const idSet = $(element).attr('id');
            $(container).find(`a[href="#${urlencodedText}"]`).attr('href', '#'+idSet);

          }else{

            $(element).attr('id', urlencodedText);
